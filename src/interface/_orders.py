...
class ScrollabFrameClients(tk.CTkScrollableFrame):
    ...
    def __init__(self, master, title, values, largura, altura, color='#1f1f1f'):
        ...
        for i, value in enumerate(self.values):
            radio = tk.CTkRadioButton(self, text=value, variable=self.radio_var, value=str(value))
...
class ButtonsFrame(tk.CTkFrame):
    ...
    def __init__(self, master, width=200, height=200, color='#1f1f1f'):
        super().__init__(master, width=width, height=height, fg_color=color)

        buttons_enviar = tk.CTkButton(self, text='Enviar')
        buttons_enviar.grid(pady=10, padx=10, column=0, row=0)

        buttons_back = tk.CTkButton(self, text='Voltar', image=IMAGE_BACK)
...
class OrderScreen(tk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        produts_view = ScrollabFrameProduct(self, title='Produtos', values=produts_names, largura=400, altura=200)
        produts_view.grid(column=0, row=0, padx=10)

        client_views = ScrollabFrameClients(self, title='Clientes', values=client_names, largura=200, altura=200)
        client_views.grid(column=1, row=0, padx=10)

        buttons_views = ButtonsFrame(self, width=600, height=200)
        buttons_views.grid(column=0, row=1, pady=10)
